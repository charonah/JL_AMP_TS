...
class AMPLoader:
    ...
    HEIGHT_START_INDEX = ANGULAR_VEL_END_IDX
    HEIGHT_END_INDEX = HEIGHT_START_INDEX + HEIGHT_SIZE
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                self.trajectories.append(torch.tensor(
                    motion_data[
                        :,
                        AMPLoader.JOINT_POSE_START_IDX:AMPLoader.HEIGHT_END_INDEX],
    ...
    def reorder_from_pybullet_to_isaac(self, motion_data):
        ...
        height = AMPLoader.get_height_batch(motion_data)
        ...
        return np.hstack(
            [joint_pos, joint_vel, lin_vel, ang_vel, height])
    
    def reorder_from_gazebo_to_isaac(self, motion_data):
        """Convert from Gazebo ordering to Isaac ordering.

        Rearranges leg and joint order from Gazebo [FL, RL, FR, RR] to
        IsaacGym order [FL, FR, RL, RR].
        """
        # root_pos = AMPLoader.get_root_pos_batch(motion_data)
        # root_rot = AMPLoader.get_root_rot_batch(motion_data)

        jp_fl, jp_rl, jp_fr, jp_rr = np.split(
            AMPLoader.get_joint_pose_batch(motion_data), 4, axis=1)
        joint_pos = np.hstack([jp_fl, jp_fr, jp_rl, jp_rr])

        jv_fl, jv_rl, jv_fr, jv_rr = np.split(
            AMPLoader.get_joint_vel_batch(motion_data), 4, axis=1)
        joint_vel = np.hstack([jv_fl, jv_fr, jv_rl, jv_rr])

        # fp_fl, fp_rl, fp_fr, fp_rr = np.split(
        #     AMPLoader.get_tar_toe_pos_local_batch(motion_data), 4, axis=1)
        # foot_pos = np.hstack([fp_fl, fp_fr, fp_rl, fp_rr])
    ...
    def get_full_frame_at_time(self, traj_idx, time):
        ...
        p = float(time) / self.trajectory_lens[traj_idx]
        n = self.trajectories_full[traj_idx].shape[0]-1
        idx_low, idx_high = int(np.floor(p * n)), int(np.ceil(p * n))
    ...
    def get_full_frame_at_time_batch(self, traj_idxs, times):
        ...
        all_frame_amp_starts = torch.zeros(len(traj_idxs), AMPLoader.HEIGHT_END_INDEX- AMPLoader.JOINT_POSE_START_IDX, device=self.device)
        all_frame_amp_ends = torch.zeros(len(traj_idxs),  AMPLoader.HEIGHT_END_INDEX - AMPLoader.JOINT_POSE_START_IDX, device=self.device)
        for traj_idx in set(traj_idxs):
            ...
            all_frame_amp_starts[traj_mask] = trajectory[idx_low[traj_mask]][:, AMPLoader.JOINT_POSE_START_IDX:AMPLoader.HEIGHT_END_INDEX]
            all_frame_amp_ends[traj_mask] = trajectory[idx_high[traj_mask]][:, AMPLoader.JOINT_POSE_START_IDX:AMPLoader.HEIGHT_END_INDEX]
        blend = torch.tensor(p * n - idx_low, device=self.device, dtype=torch.float32).unsqueeze(-1)
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        ...
        for _ in range(num_mini_batch):
            if self.preload_transitions:
                ...
                s = self.preloaded_s[idxs, AMPLoader.JOINT_POSE_START_IDX:AMPLoader.HEIGHT_END_INDEX]
                # s = torch.cat([
                #     s,
                #     self.preloaded_s[idxs, AMPLoader.ANGULAR_VEL_END_IDX ]], dim=-1)
                s_next = self.preloaded_s_next[idxs, AMPLoader.JOINT_POSE_START_IDX:AMPLoader.HEIGHT_END_INDEX]
                # s_next = torch.cat([
                #     s_next,
                #     self.preloaded_s_next[idxs, AMPLoader.ANGULAR_VEL_END_IDX]], dim=-1)
            else:
                s, s_next = [], []
    ...
    def set_frame_duration(self, duration = 0.21):
        self.trajectory_frame_durations = torch.ones_like(self.trajectory_frame_durations) * duration
    ...
    @staticmethod
    def get_height(pose):
        return pose[AMPLoader.HEIGHT_START_INDEX:AMPLoader.HEIGHT_END_INDEX]  

    @staticmethod
    def get_height_batch(poses):
        return poses[:, AMPLoader.HEIGHT_START_INDEX:AMPLoader.HEIGHT_END_INDEX]  
