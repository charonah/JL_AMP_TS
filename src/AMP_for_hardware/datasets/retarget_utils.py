...
def get_joint_limits(robot):
  ...
  for i in range(num_joints):
    ...
    if (joint_type == pybullet.JOINT_PRISMATIC or
        joint_type == pybullet.JOINT_REVOLUTE):
...
# jointIndex: 关节的索引。
# jointName: 关节的名称（字符串）。
# jointType: 关节的类型，通常是以下类型之一：
# JOINT_REVOLUTE (0): 旋转关节
# JOINT_PRISMATIC (1): 平移关节
# JOINT_SPHERICAL (2): 球形关节
# JOINT_PLANAR (3): 平面关节
# JOINT_FIXED (4): 固定关节
# qIndex: 关节的位置索引，表示在状态空间（state space）中的位置直接在getJointState中使用。
# uIndex: 关节的速度索引。
# flags: 关节的标志，包括是否允许运动（JOINT_HAS_MOTOR, JOINT_LIMITED等）。
# maxForce: 最大施加力的值（如果适用）。
# maxVelocity: 最大速度（如果适用）。
# jointDamping: 关节阻尼参数。
# jointGearRatio: 齿轮比值（如果适用）。
# jointLowerLimit: 关节的最小限制值（如果适用）。
# jointUpperLimit: 关节的最大限制值（如果适用）。
# jointTypeName: 关节类型的字符串描述。
...
def set_pose(robot, pose):
  ...
  for j in range(num_joints):
    ...
    print(j_info[0],"name: ",j_info[1])
    j_pose_idx = j_info[3] # 关节位置索引
...
# def output_motion_31(frames, out_filename, motion_weight, frame_duration):
#   with open(out_filename, "w") as f:
#     f.write("{\n")
#     f.write("\"LoopMode\": \"Wrap\",\n")
#     f.write("\"FrameDuration\": " + str(frame_duration) + ",\n")
