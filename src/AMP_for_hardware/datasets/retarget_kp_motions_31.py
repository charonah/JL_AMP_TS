...
# from datasets import retarget_config_a1_31 as config
import retarget_config_ysc1_31 as config
...
REF_TOE_JOINT_IDS = [10, 15, 19, 23]  #足在keypoint中的位置
...
def retarget_pose(robot, default_pose, ref_joint_pos):
  ...
  # print('root_pos1:', root_pos)

  root_pos += config.SIM_ROOT_OFFSET
  # print('root_pos2:', root_pos)
...
def retarget_motion(robot, joint_pos_data):
  ...
  time_between_frames = FRAME_DURATION #两个frame之间的时间间隔
  ...
  for f in range(num_frames - 1):
    ...
    # joint pos + joint vel +base line vel+ angular vel + height
    data1 = curr_pose[7:7+12]
    data2 = curr_pose[2:3]
    # print(data2)
    # print(curr_pose)
    # print(data1.shape)
    ...
    # print(del_linear_vel.shape)
    # print(np.shape(del_angular_vel))
    # print(data1)

    # root_pos3,root_rot4,jointpos12,tar_toe_pos_local12,line3,ang3,jointvel12,toe12 
    curr_pose = np.concatenate([
        curr_pose, del_linear_vel, del_angular_vel, joint_velocity, toe_velocity
    ])
    # [7:7+12],[37:37+12], [31:31+3], [34:34+3],[2:3]
    # jointpos12, jointvel12, line3, ang3, height1
    # curr_poses = np.concatenate([data1, joint_velocity, del_linear_vel, del_angular_vel, data2])
...
def retarget_motion_31(robot, joint_pos_data):
  num_frames = joint_pos_data.shape[0]
...
def main(argv):
  ...
  for mocap_motion in config.MOCAP_MOTIONS:
    ...
    for i in range(joint_pos_data.shape[0]):
      # 对每个样本所有keypoint位置进行旋转坐标系，旋转根节点，以及缩放+offset
      joint_pos_data[i] = process_ref_joint_pos_data(joint_pos_data[i])

    # 对keypoint数据集中的足的位置高度进行归一化，并加上偏移量(应该是足的半径)
    ...
    retarget_frames_31 = retarget_motion_31(robot, joint_pos_data)
    joint_pos_data = joint_pos_data[:-1, :]

    # output 路径
    output_file = os.path.join(output_dir, f"{mocap_motion[0]}.txt")
    # 保存retarget_frames到output文件中，
    # output_motion(retarget_frames, output_file, mocap_motion[4], FRAME_DURATION)
    output_motion(retarget_frames_31, output_file, mocap_motion[4], FRAME_DURATION)
